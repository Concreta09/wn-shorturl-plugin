...
use Config;
use Concreta\ShortURL\Controllers\ShortURLController;
...
class Builder
{
    ...
    {
        return Config::get('concreta.shorturl::middleware', []);
    }
    ...
    {
        if ($this->secure === null) {
            $this->secure = Config::get('concreta.shorturl::enforce_https');
        }
        ...
        if ($this->forwardQueryParams === null) {
            $this->forwardQueryParams = Config::get('concreta.shorturl::forward_query_params') ?? false;
        }
    ...
    private function setTrackingOptions(): void
    {
        if ($this->trackVisits === null) {
            $this->trackVisits = Config::get('concreta.shorturl::tracking.default_enabled');
        }

        if ($this->trackIPAddress === null) {
            $this->trackIPAddress = Config::get('concreta.shorturl::tracking.fields.ip_address');
        }

        if ($this->trackOperatingSystem === null) {
            $this->trackOperatingSystem = Config::get('concreta.shorturl::tracking.fields.operating_system');
        }

        if ($this->trackOperatingSystemVersion === null) {
            $this->trackOperatingSystemVersion = Config::get('concreta.shorturl::tracking.fields.operating_system_version');
        }

        if ($this->trackBrowser === null) {
            $this->trackBrowser = Config::get('concreta.shorturl::tracking.fields.browser');
        }

        if ($this->trackBrowserVersion === null) {
            $this->trackBrowserVersion = Config::get('concreta.shorturl::tracking.fields.browser_version');
        }

        if ($this->trackRefererURL === null) {
            $this->trackRefererURL = Config::get('concreta.shorturl::tracking.fields.referer_url');
        }

        if ($this->trackDeviceType === null) {
            $this->trackDeviceType =Config::get('concreta.shorturl::tracking.fields.device_type');
        }
    }
    ...
    {
        $baseUrl = Config::get('concreta.shorturl::default_url') ?? config('app.url');
        $baseUrl .= '/';
