...
A [Short URL Laravel package](https://github.com/ash-jc-allen/short-url) adapted for WinterCMS that can be used for adding shortened URLs to your existing web app.
...
``` \Concreta\ShortURL\Controllers\ShortURLController ```.
...
```php
Route::get('/custom/{shortURLKey}', '\Concreta\ShortURL\Controllers\ShortURLController');
```
...
You can't remove the prefix from the default route, if you set the `prefix` config value to `null` or empty string the default prefix `short` will be used:
...
each of these will be converted to "short".
...
```php
\Concreta\ShortURL\Facades\ShortURL::routes();
```
...
```php
$shortURL = \Concreta\ShortURL\Models\ShortURL::find(1);
...
```php
$shortURL = \Concreta\ShortURL\Models\ShortURL::findByKey('abc123');
```
...
```php
$shortURLs = \Concreta\ShortURL\Models\ShortURL::findByDestinationURL('https://destination.com');
```
...
```php
$shortURL = \Concreta\ShortURL\Models\ShortURL::first();
...
```php
'factories' => [
    \Concreta\ShortURL\Models\ShortURL::class => \Concreta\ShortURL\Models\Factories\ShortURLFactory::class,
    \Concreta\ShortURL\Models\ShortURLVisit::class => \Concreta\ShortURL\Models\Factories\ShortURLVisitFactory::class
],
```
...
```
Concreta\ShortURL\Events\ShortURLVisited
```
...
If you've found this package useful, please consider support [WinterCMS](https://github.com/sponsors/LukeTowers).
